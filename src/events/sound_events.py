"""소리 이벤트(기능 3) — 지금은 '이름 호명'만 음성 기반으로 감지한다.

왜 이렇게(경량 구현):
  원래 기능 3은 종소리·경보 같은 '말이 아닌 소리'까지 오디오 모델로 분류하는 것이었다.
  그건 모델·시간이 필요해 다음 단계로 남기고, 먼저 이름 호명만 안전하게 구현한다.
  방법: 자막(STT)이 이미 뽑아 준 텍스트에 학생 이름이 들어 있으면 알림을 낸다.
  → 새 모델·마이크 0. 자막 텍스트를 '읽기'만 한다.
"""
